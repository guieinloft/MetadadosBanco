...
def main():
    example = Cliente(generate_random_value('int(11)'), generate_random_value('varchar(255)'), generate_random_value('varchar(20)'), generate_random_value('date'))

    dao = ClienteDAO()
    # Inserir exemplo
    dao.inserir(example.getidCliente(), example.getnome(), example.getcpf(), example.getnasc())
    print(f'{example} foi inserido')

    # Buscar exemplo
    buscado = dao.buscar(example.getidCliente())
